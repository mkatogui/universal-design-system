...
const CHECKS: Check[] = [
  ...
  },
  {
    ...
    fn: () => {
      ...
      try {
        ...
        return hasTheme ? `valid JSON, ${paletteCount} palettes in theme` : "valid JSON but missing theme section";
      } catch {
  ...
  },
  {
    ...
    fn: () => {
      if (!fileExists("tokens/palette-registry.json")) return "MISSING: tokens/palette-registry.json";
      try {
  ...
  },
  {
    ...
    fn: () => {
      ...
      const expected = [
        "products.csv", "components.csv", "ui-reasoning.csv", "anti-patterns.csv",
        "typography.csv", "styles.csv", "colors.csv", "patterns.csv",
        "google-fonts.csv", "icons.csv", "landing.csv", "charts.csv",
        "ux-guidelines.csv", "app-interface.csv", "react-performance.csv",
      ];
  ...
  },
  {
    ...
    fn: () => {
      const scripts = ["scripts/validate-tokens.py", "scripts/wcag-audit.py", "scripts/verify-docs.py"];
...
export async function doctorCommand(): Promise<void> {
  ...
  for (const check of CHECKS) {
    ...
    if (result === true) {
      console.log("✓ ok");
    } else if (result.startsWith("MISSING") || result.startsWith("INVALID") || result.includes("not found") || result.includes("not installed") || result.includes("not built")) {
      console.log(`✗ ${result}`);
      issues++;
    } else {
      console.log(`✓ ${result}`);
