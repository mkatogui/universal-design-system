...
| Field | Detail |
|-------|--------|
| **What is promised** | All slug references between the 17 CSV databases resolve correctly. For example, `key_components` in `src/data/products.csv` must reference valid slugs in `src/data/components.csv`. No dangling references. |
