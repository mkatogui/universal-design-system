    ...
    return createPortal(
      <div className="uds-modal-overlay" onClick={(e) => { if (e.target === e.currentTarget) onClose(); }} role="presentation">
        <div
          ...
          className={`uds-modal ${className}`.trim()}
          role="dialog"
          aria-modal="true"
          aria-label={title}
        >
          {title && (
            <div className="uds-modal__header">
              ...
              <button className="uds-modal__close" onClick={onClose} aria-label="Close dialog">
          ...
          {footer && <div className="uds-modal__footer">{footer}</div>}
        </div>
      </div>,
      document.body
    );
