 ...
 *
 * This is a standalone build that reads design-tokens.json directly.
 * The root-level Style Dictionary config (style-dictionary.config.mjs)
...
function extractPaletteOverrides(resolvedMap) {
  ...
  for (const [dotPath, val] of resolvedMap) {
    ...
    if (themeMatch) {
      ...
      const normalized = tokenTail.replaceAll('_', '-');
  ...
  for (const [dotPath, val] of resolvedMap) {
    ...
    if (darkFlatMatch) {
      ...
      const normalized = tokenTail.replace(/_dark$/, '').replaceAll('_', '-');
      const cssVar = `--color-${normalized}`;
      darkMap.set(cssVar, toCssValue(val));
    }
  }
...
function generateCSS(resolvedMap) {
  ...
  for (const name of PALETTES) {
    ...
    lines.push(`  /* Palette: ${name} (light) */`, `  [data-theme="${name}"] {`);
...
function generateTypeDeclarations(varRefObj) {
  ...
  function typeFromObj(obj, indent = '') {
    ...
    const inner = entries.map(([key, val]) => {
      ...
      if (typeof val === 'object' && val !== null && !Array.isArray(val)) {
        return `${childIndent}readonly ${safeKey}: ${typeFromObj(val, childIndent)}`;
      }
      return `${childIndent}readonly ${safeKey}: string`;
    });
  ...
  lines.push(
    `export declare const tokens: ${typeFromObj(varRefObj)};`,
    '',
...
function generateTokenTypes(resolvedMap) {
  ...
  lines.push(
    ...
    ` * Total: ${rootTokenNames.length} tokens.`,
    ' */',
  ...
  lines.push(
    ...
    ...paletteKeys.map((key) => `  readonly '${key}': string;`),
    '}',
    '',
  );
...
function objToJSLiteral(obj, indent = '  ') {
  ...
  const lines = entries.map(([key, val]) => {
    ...
    if (typeof val === 'object' && val !== null && !Array.isArray(val)) {
      const childIndent = `${indent}  `;
      return `${indent}${safeKey}: ${objToJSLiteral(val, childIndent)}`;
    }
    // String value
    const escaped = String(val).replaceAll('\\', '\\\\').replaceAll("'", "\\'");
